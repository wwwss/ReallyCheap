...
public class ShoppingCartFragment extends BaseFragment implements ShoppingCartView, View.OnClickListener {
    ...
    public void updateTotalPrice(double productsPrice) {
        ...
        String sendPrice = LocalCache.get(context).getAsString("send_price");
        if (TextUtils.isEmpty(sendPrice))
            return;
        if (productsPrice < Double.parseDouble(sendPrice)) {
            double freight = Double.parseDouble(sendPrice) - totalPrice;
            this.freight = freight;
            tvTotalHint.setText("(包含运费" + freight + "元)");
            totalPrice += freight;
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.selectShippingAddress:
                ...
                break;
            case R.id.operation:
                shoppingCartPresenter.goSettlement(shoppingCartBeans, shippingAddressBean, productsPrice, totalPrice, freight);
                break;
            case R.id.shoppingCartEmptyHint:
                onTabSwitchToListener.onTabSwitch(0, null);
                break;
        }
